...
Genius only marks sections with sung lyrics, so anything purely instrumental is invisible and `init-manifest` won't emit it. When there's a gap of `≥ 2 bars` between two lyric sections (or before the first / after the last), suggest an instrumental section and ask the user:
